...
def detailed_post(userID: int):
    ...
    # Structure holding posts and comments
    data = {"userID": userID, "posts": []}
    ...
    for post in posts:
        if post['userId'] == userID:
            ...
            post_comments = requests.get(f'https://jsonplaceholder.typicode.com/comments?postId={post["id"]}')
            comments = json.loads(post_comments.text)
            
            # Add the post and its comments to the data
            data["posts"].append({
                ...
                "comments": [
                    ...
                    for comment in comments  # Add all comments related to this post
                ]
            })
